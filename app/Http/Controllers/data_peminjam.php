...
class data_peminjam extends Controller
{
    public function rentlogs(Request $request){
        if ($request) {
            $data = rentlogs::where('nama','LIKE','%'.$request->cari2.'%')
            ->orWhere('email', 'LIKE', '%'. $request->cari2.'%')
            ->orWhere('JudulBuku', 'LIKE', '%'. $request->cari2.'%')
            ->orWhere('kode', 'LIKE', '%'. $request->cari2.'%')
            ->paginate(6);
        } else {
            $data = rentlogs::all()->paginate(6);
        }
        // dd($data);
