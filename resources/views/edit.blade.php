        ...
        <form  class=" fixed px-10 py-14 rounded-xl bg-black bg-opacity-80 " action="/{{$edit->id}}" method="POST">
          ...
          @csrf
            ...
            <select id="select" class="cursor-pointer block mt-5 rounded-md focus:outline-none p-1" name="status">
                ...
                <option value="0" <?php if($edit->status == "1"){echo "selected";}?> class="bg-red-200 text-red-500 font-semibold" >Dipinjam</option>
                <option value="1" <?php if($edit->status == "0"){echo "selected";}?> class="bg-green-200 text-green-500 font-semibold">Tersedia</option>
            </select>
            <div class="">
